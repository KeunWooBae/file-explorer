...
function registerAssets(){
 const assets=new Map([['index.html','text/html'],['styles.css','text/css'],['ssh-workspace.css','text/css'],['app.js','text/javascript'],['ssh-workspace.js','text/javascript'],['theme.js','text/javascript'],['node_modules/@xterm/xterm/lib/xterm.js','text/javascript'],['node_modules/@xterm/xterm/css/xterm.css','text/css'],['node_modules/@xterm/addon-fit/lib/addon-fit.js','text/javascript']]);
...
const sftpService=createSftpService({directory:dataDirectory,
 ...
 onTerminal:value=>mainWindow?.webContents.send('pane:ssh-output',value),
 onState:value=>mainWindow?.webContents.send('pane:ssh-state',value)
});
...
function registerFileAccess() {
  ...
  handle('pane:ssh-key-picker',async()=>{const result=await dialog.showOpenDialog(mainWindow,{title:'SSH 개인키 선택',properties:['openFile']});return result.canceled?null:result.filePaths[0];});
...
function createWindow() {
  ...
  window.on('closed', () => { sftpService.disconnect().catch(()=>{});mainWindow = undefined; });
