...
export function initSshWorkspace(getLocalPath) {
  ...
  async function openTerminal(){
    ...
    try{setupTerminal();term.clear();unwrap(await api.sshOpen({cols:Math.min(500,term.cols),rows:Math.min(200,term.rows)}));terminalReady=true;resize();term.focus();}
  ...
  nav.onclick=show;
  $('.sidebar').addEventListener('click',event=>{if(event.target.closest('button')&&!event.target.closest('#ssh-menu')){screen.hidden=true;original.hidden=false;nav.classList.remove('selected');previousSelection?.classList.add('selected');}});
  ...
  const form=$('#ssh-form');
  form.elements.authMode.onchange=()=>{const mode=form.elements.authMode.value;$('#ssh-key-fields').hidden=mode!=='key';$('#ssh-password-field').hidden=mode!=='password';$('#ssh-passphrase-field').hidden=mode==='password';};
  $('#ssh-key-browse').onclick=async()=>{try{const file=unwrap(await api.sshKeyPicker());if(file)form.elements.privateKeyPath.value=file;}catch(e){$('#ssh-form-error').textContent=e.message;}};
  form.onsubmit=async event=>{
    event.preventDefault();if(connecting)return;connecting=true;controls();
    const input=Object.fromEntries(new FormData(form));if(input.authMode!=='key')delete input.privateKeyPath;if(input.authMode!=='password')delete input.password;
    for(const item of form.elements)item.disabled=true;$('#ssh-form-error').textContent='연결 중…';
    try{const session=unwrap(await api.sftpConnect(input));connected=true;remote.path=session.path;$('#ssh-connection-label').textContent=`${session.username}@${session.host}:${session.port}`;dialog.close();status('연결되었습니다. 항목을 선택한 뒤 업로드 또는 다운로드를 누르세요.');await Promise.all([load('remote',session.path),local.path?Promise.resolve():load('local',getLocalPath())]);await openTerminal();}
  ...
  function lost(){connected=false;terminalReady=false;remote.ticket++;remote.loading=false;remote.entries=[];remote.path='';remote.selected.clear();draw('remote');$('#ssh-connection-label').textContent='연결되지 않음';status('SSH 연결이 종료되었습니다.');controls();}
  ...
  async function transfer(direction){
    ...
    finally{busy=false;await Promise.all([load('local',local.path),connected?load('remote',remote.path):Promise.resolve()]);controls();}
  }
  ...
  select.onchange=()=>{try{localStorage.setItem('pane-font-size',select.value);}catch{}applyFont();};applyFont();
  controls();
}
